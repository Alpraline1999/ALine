...
class TestAnalysisPageBoolRuntimeUI(unittest.TestCase):
    ...
    def test_builtin_extension_run_uses_selected_inputs_when_lines_list_is_empty(self) -> None:
        ...
        def _selection_probe(lines, _params):
            return {
                "analysis_type": "selected_input_probe",
                "summary_items": [("line_count", len(lines))],
            }

        extension_registry.register_analysis(
            AnalysisExtension(
                type="selected_input_probe",
                name="选中输入探针",
                handler=_selection_probe,
                lines_number=(1, -1),
            )
        )
        ...
        try:
            ...
            self.page._type_combo.setCurrentIndex(self.page._analysis_type_ids.index("selected_input_probe"))
            ...
            self.assertIsInstance(self.page._result, dict)
            self.assertEqual(self.page._result.get("analysis_type"), "selected_input_probe")
            summary_items = self.page._result.get("summary_items") or []
            self.assertTrue(summary_items)
        finally:
            extension_registry.unregister_analysis("selected_input_probe")
