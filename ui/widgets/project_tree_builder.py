...
class ProjectTreeBuildOwner(Protocol):
    ...
    def _apply_focus_view(self, selected_keys: Optional[list[str]]) -> list[str]: ...
    def _restore_selection(self, selected_key: Optional[list[str] | str]) -> None: ...
...
class ProjectTreeBuilder:
    def build(self, owner: ProjectTreeBuildOwner) -> None:
        ...
        selected_items = getattr(owner, "_selected_items_or_current", None)
        selected_item_keys = getattr(owner, "_selected_item_keys", None)
        if callable(selected_items) and callable(selected_item_keys):
            selected_key = selected_item_keys(selected_items())
        else:
            current_key = owner._current_item_key()
            selected_key = [current_key] if current_key else []
