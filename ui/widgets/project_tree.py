...
class ProjectTreeWidget(QWidget):
    ...
    def _build_children(
        ...
        for node in children:
            ...
            # 过滤：文件夹下无可见子节点则隐藏
            if kind == "folder" and self._filter_kinds and item.childCount() == 0:
                if parent_item is None:
    ...
    def _on_context_menu(self, pos) -> None:
        ...
        if item is None:
            act_new = menu.addAction(FIF.FOLDER.icon(), "新建文件夹")
            act_new.triggered.connect(lambda: self._cmd_new_folder(None))
        ...
        if kind == "folder":
            ...
            is_protected = group_type in _PROTECTED_GROUP_TYPES
            menu.addAction(FIF.FOLDER.icon(), "新建子文件夹").triggered.connect(
                lambda: self._cmd_new_folder(node_id))
            if not is_protected:
                menu.addSeparator()
    ...
    def _cmd_new_folder(self, parent_id: Optional[str]) -> None:
        name, ok = QInputDialog.getText(self, "新建文件夹", "文件夹名称:")
        if ok and name.strip():
            project_manager.add_folder(name.strip(), parent_id, group_type="user")
            self.refresh()
            self.project_modified.emit()
