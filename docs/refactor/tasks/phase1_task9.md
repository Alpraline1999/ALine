...
- 继续扩展共享树命令服务。
- 本次纳入单项移动命令 `_cmd_move_virtual`。

## 本任务范围

- 扩展 `ProjectTreeCommandService.move_virtual(...)`。
- 让 `ProjectTreeWidget` 将 `_cmd_move_virtual` 委托给服务执行。
- 补对应窄测。
...
- 单项移动命令不再由 `ProjectTreeWidget` 直接承载核心执行逻辑。
