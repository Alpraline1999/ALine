...
from core.line_tools import x_values_equal
from extensions.processing.extension_tools import BUILTIN_EXTENSION_VERSION, line_from_xy, line_xy, normalize_lines
...
def pairwise_compute_handler(lines, params):
    ...
    primary, secondary = input_lines
    x1, y1 = line_xy(primary)
    x2, y2 = line_xy(secondary)
    if not x_values_equal(x1, x2):
        raise ValueError("双曲线运算要求两条输入曲线的 X 网格完全一致；请先在 pipeline 中进行坐标间距重采样")
        ...
        ProcessingExtension(
            ...
            config_fields=[
                ExtensionConfigField(
                    ...
                    default="",
                    choices=["", "add", "subtract", "multiply", "divide", "abs_diff"],
                    description="选择后自动填充下方表达式；留空则手动输入。",
                ),
