...
def _dashed_trace(figure: str, params):
    ...
    step = max(1, int(options.get("step", 3) or 3))
    mask = apply_mask_polygons(mask, options.get("mask_polygons"), bool(options.get("mask_include_mode", True)))
    points = trace_mask_columns(mask, step=step, y_mode=str(options.get("y_mode", "median") or "median"))
    points = fill_point_gaps(points, max_gap=max(float(options.get("max_gap", 24.0) or 24.0), float(step)), step=step)
    points = smooth_point_series(points, window=max(1, int(options.get("smooth_window", 3) or 3)))
...
def register_extensions(registry) -> None:
    registry.register_digitize(
        DigitizeExtension(
            type="builtin_digitize_dashed_trace",
            name="虚线曲线追踪",
            handler=_dashed_trace,
            description="对虚线或点划线按列追踪，并在小间断内自动线性补点。",
            ...
            config_fields=[
                ...
                ExtensionConfigField(key="max_gap", label="最大补点间隙", field_type="number", default=24.0, min_value=1.0, step=1.0),
                ExtensionConfigField(key="y_mode", label="列内 Y 统计", field_type="selective", default="median", choices=("median", "mean")),
                ExtensionConfigField(key="smooth_window", label="平滑窗口", field_type="integer", default=3, min_value=1, max_value=31),
