...
- `ProcessingExtension`: 兼容旧签名 `(xs, ys, params) -> (xs, ys)`；新扩展可额外接收 `lines` 参数读取完整已选择列表
- `AnalysisExtension`: 兼容旧签名 `(inputs, params) -> result_dict`；新扩展可额外接收 `lines_list` 参数，且第一项始终是主曲线
...
- `processing_multi_curve_mean_demo.py`: 处理扩展，演示多曲线自动对齐后求均值
- `analysis_spectrum_demo.py`: 分析扩展，返回频谱结果、主频和可绘制曲线
- `analysis_multi_curve_correlation_demo.py`: 分析扩展，演示以主曲线为基准的多曲线相关性比较
- `plot_reference_line_demo.py`: 绘图扩展，添加参考线和注释
- `plot_dual_curve_band_demo.py`: 绘图扩展，演示双曲线差异带与最大差值标注
...
此时扩展 handler 可以通过 `lines` 读取完整已选择列表；如果启用了多曲线工具，处理页会做输入数量校验，并避免同一条处理链里叠加多个多曲线步骤。
...
如果你的分析扩展希望显式读取“主曲线 + 其余输入”的顺序，可定义：

```python
def analyze(inputs, params, lines_list=None):
    primary = (lines_list or inputs)[0]
    ...
```
...
def mean_curve(xs, ys, params, lines=None):
    ...
...
- 多曲线处理：`processing.data_engine.apply_pipeline_to_lines([...], [{"type": "multi_curve_mean", "params": {...}}])`
