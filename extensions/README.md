...
- `PlotExtension`: 绘图扩展，兼容旧签名 `(axis, series, options) -> None`，也支持新的上下文签名 `(plot_context, options) -> None`
...
`PlotExtension` 使用新上下文签名时，`plot_context` 会提供：

- `figure` / `canvas`: 当前 matplotlib Figure 与画布
- `axis` / `axes`: 当前主轴与 Figure 中全部轴
- `visible_series` / `plotted_series`: 可见曲线源数据与默认绘制后的曲线摘要
- `figure_state` / `plot_style_extras` / `theme_colors`: 当前绘图配置、扩展样式附加项和主题色
- `phase`: `before_plot` 或 `after_plot`
- `skip_default_plot`: 设为 `True` 后跳过内置单轴折线绘制
- `skip_default_formatting`: 设为 `True` 后跳过内置坐标轴/图例/文字样式设置
- `skip_default_layout`: 设为 `True` 后跳过默认 `tight_layout/subplots_adjust`

这意味着扩展可以自行 `figure.clear()`、创建多个 subplot、极坐标轴、inset axes、colorbar，或者完全接管整张图的 matplotlib 绘制流程。
